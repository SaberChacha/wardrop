...
// Use environment variable or default to /api for relative URLs
...
export const clothingAPI = {
  ...
  deleteImage: async (clothingId: number, imageId: number) => {
    const response = await api.delete(
      `/clothing/${clothingId}/images/${imageId}`
    );
    return response.data;
  },
  delete: async (id: number) => {
...
export const exportAPI = {
  ...
  exportClothing: async () => {
    const response = await api.get("/export/clothing", {
      responseType: "blob",
    });
    return response.data;
  },
  exportBookings: async (params?: {
    ...
    const response = await api.get("/export/bookings", {
      responseType: "blob",
      params,
    });
    return response.data;
