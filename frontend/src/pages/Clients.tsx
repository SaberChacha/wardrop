...
export default function Clients() {
  ...
  const handleClientTap = useCallback((client: any, e: React.TouchEvent) => {
    const now = Date.now();
    if (
      lastTapRef.current.id === client.id &&
      ...
      lastTapRef.current = { id: client.id, time: now };
    }
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <ClientForm client={editingClient} onSuccess={handleCloseModal} />
      </Modal>
